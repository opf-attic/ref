...
function perform_scan($path,$tool_id) {
	$now = time();
	$report_file1 = "/tmp/droid5.0.3-39_$now.droid";
	$report_file2 = "/tmp/droid5.0.3-39_$now.csv";

	require_once('../java_path.php');
	$java_path = get_java_path_droid_5();
	
	$cmd = $java_path . 'java -jar droid-command-line-5.0.3.jar -s 39 2>/dev/null >/dev/null';
