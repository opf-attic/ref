...
function perform_scan($path,$tool_id) {
	$now = time();
	$report_file = "/tmp/tika_$now.txt";
	$cmd = "java -jar tika-app-1.2.jar -j $path* > $report_file";
	$ret = exec($cmd,$output);
...
function process_results($path,$tool_id,$report_file) {
	...
	for($i=0;$i<count($decoded);$i++) {
		...
		$mime = $array["Content-Type"];
		$triples["hasMime"] = $array["Content-Type"];

		if (strpos($mime,";") > 0) {
                        $partsx = explode(";",$mime);
                        $triples["hasMime"] = trim($partsx[0]);
                        $triples["hasCharset"] = trim($partsx[1]);
			if (substr($triples["hasMime"],strlen($triples["hasMime"])-1,strlen($triples["hasMime"])) == ":") {
                        	$triples["hasMime"] = substr($triples["hasMime"],0,strlen($triples["hasMime"])-1);
			}
                }
                $partx2 = explode(" ",$mime);
                if (!$info["hasCharset"] && count($partx2) > 1) {
                        $triples["hasMime"] = $partx2[0];
                        $triples["hasCharset"] = $partx2[1];
                }

		$filename = $array["resourceName"];
		$rel_path = substr($path,strrpos($path,"govdocs_selected/"),strlen($path)) . $filename;
